...
import {
  DomSanitizer
} from "./chunk-ZFQRDC55.js";
import {
  HttpClient
} from "./chunk-LSOYFJ6M.js";
import {
  DOCUMENT
...
var _MatIconRegistry = class _MatIconRegistry {
  ...
  _getSvgFromIconSetConfigs(name, iconSetConfigs) {
    ...
    const iconSetFetchRequests = iconSetConfigs.filter((iconSetConfig) => !iconSetConfig.svgText).map((iconSetConfig) => {
      return this._loadSvgIconSetFromConfig(iconSetConfig).pipe(catchError((err) => {
        ...
        const errorMessage = "Loading icon set URL: ".concat(url, " failed: ").concat(err.message);
  ...
  _extractSvgIconFromSet(iconSet, iconName, options) {
    const iconSource = iconSet.querySelector('[id="'.concat(iconName, '"]'));
    if (!iconSource) {
      return null;
    }
...
var funcIriAttributeSelector = funcIriAttributes.map((attr) => "[".concat(attr, "]")).join(", ");
  ...
  _updateSvgIcon(rawName) {
    ...
    if (rawName) {
      ...
      this._currentIconFetch = this._iconRegistry.getNamedSvgIcon(iconName, namespace).pipe(take(1)).subscribe((svg) => this._setSvgElement(svg), (err) => {
        const errorMessage = "Error retrieving icon ".concat(namespace, ":").concat(iconName, "! ").concat(err.message);
        this._errorHandler.handleError(new Error(errorMessage));
      });
    }
  }
